...
void AP_LTM::get_position(void){
    ...
    if(_ahrs.get_position(loc)) {
		...
		_uav.alt = (int32_t)roundf(alt*100.0f); 
...
void AP_LTM::get_sense(void){ 
    ...
    if (aspeed && aspeed->enabled())  _uav.airspeed = (uint8_t)roundf(aspeed->get_airspeed()); 
...
void AP_LTM::get_attitude(void){	
	_uav.pitch = (int16_t)(_ahrs.pitch_sensor / 100);
	if(_uav.pitch > 180) _uav.pitch -= 360;
	if(_uav.pitch < 180) _uav.pitch += 360;
	_uav.roll = (int16_t)(_ahrs.roll_sensor / 100);
	if(_uav.roll > 180) _uav.roll -= 360;
	if(_uav.roll < 180) _uav.roll += 360;
	_uav.heading = (int16_t)(_ahrs.yaw_sensor / 100);
	if(_uav.heading > 180) _uav.heading -= 360;
	if(_uav.heading < 180) _uav.heading += 360;
}	
